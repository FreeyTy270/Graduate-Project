...
#define fg 783.99
#define fe 659.25
#define TOREAL 3.21/4096
#define TIM_PERIOD 1e6/FS
/* USER CODE END PD */
...
//				fft_res.res_buf[i] = ((sin(2*PI*fc*i/FS)) + (sin(2*PI*fg*i/FS)) + (sin(2*PI*fe*i/FS))); // C Chord triad
			...
			temp_buf = malloc((SMP_2+1)*sizeof(float));
			...
			for(int i = 0; i < (SMP_2+1); i++)
			{
				...
				temp_buf[i] = creal(fft_res.res_buf[i])*2; // Correct windowed amplitude and transfer to smaller buffer

				if(temp_buf[i] > 9 && temp_buf[i] < 100)
					sprintf(pBuf+i*STR_SZ, "%.5f,", temp_buf[i]);

				else if(temp_buf[i] > 99)
					sprintf(pBuf+i*STR_SZ, "%.4f,", temp_buf[i]);
